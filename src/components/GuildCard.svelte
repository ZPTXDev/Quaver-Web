	...
	<a href={getGuildURL(guild)} class="flex flex-col gap-1 text-900 hover:background-200 focus:background-200 font-semibold w-full p-3 transition rounded-2xl truncate">
		<div class="rounded-xl relative h-32 w-full overflow-hidden">
			{#if guild?.banner || guild?.icon}
				<img draggable="false" ondragstart={(e) => {e.preventDefault(); return false;}} src={guild.banner ? getGuildBannerURL(guild) : getGuildIconURL(guild)} alt={guild.name} class="pointer-events-none drag-none absolute h-32 w-full blur-sm {guild.banner ? '' : ' object-none scale-150'}" />
				<img draggable="false" ondragstart={(e) => {e.preventDefault(); return false;}} src={getGuildIconURL(guild)} alt={guild.name} class="pointer-events-none drag-none absolute h-20 w-20 object-cover rounded-xl top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
			{:else}
